...
class Catalog extends Entity{
	...
	protected function _customUploadImg($file = array()){
        if( is_array($file) || is_object($file) ){
            ...
            $oldPath = WWW_ROOT.'img'.DS.'catalogs'.DS.$fileName;
            $oldPathThumb = WWW_ROOT.'img'.DS.'catalogs'.DS.'thumbs'.DS.$fileName;
            ...
            $path = WWW_ROOT . 'img'.DS.'catalogs' .DS. $fileName;
            $path_th = WWW_ROOT . 'img'.DS.'catalogs'.DS.'thumbs'.DS. $fileName;
            $path_orig = WWW_ROOT . 'img'.DS.'catalogs'.DS.'original'.DS. $fileName;

            $file_move = $file->moveTo($path_orig);
            $this->_resizeImg($path_orig, $path_th, 300, 300, $ext);
            $this->_resizeImg($path_orig, $path, 1500, 1500, $ext);
    ...
    protected function _genNameFile($ext){
        ...
        if(is_file(WWW_ROOT . 'img'.DS.'catalogs'.DS. $name)){
    ...
    public function _setImages($file){
        if( is_string($file) ){
