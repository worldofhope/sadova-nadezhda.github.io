...
class Quality extends Entity{
	...
	protected function _customUploadImg($file = array()){
		if( is_array($file) || is_object($file) ){
			...
			$oldPath = WWW_ROOT.'img'.DS.'qualities'.DS.$fileName;
			$oldPathThumb = WWW_ROOT.'img'.DS.'qualities'.DS.'thumbs'.DS.$fileName;
			...
			$path = WWW_ROOT . 'img'.DS.'qualities' .DS. $fileName;
			$path_th = WWW_ROOT . 'img'.DS.'qualities'.DS.'thumbs'.DS. $fileName;
			$path_orig = WWW_ROOT . 'img'.DS.'qualities'.DS.'original'.DS. $fileName;

			$file_move = $file->moveTo($path_orig);
			$this->_resizeImg($path_orig, $path_th, 100, 100, $ext);
			$this->_resizeImg($path_orig, $path, 200, 200, $ext);
	...
	protected function _genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img'.DS.'qualities'.DS. $name)){
	...
	protected function _customUploadImgProd($file = array()){
		if( is_array($file) || is_object($file) ){
			$fileName = $file->getClientFilename();
			$oldPath = WWW_ROOT.'img'.DS.'qualities'.DS.$fileName;
			$oldPathThumb = WWW_ROOT.'img'.DS.'qualities'.DS.'thumbs'.DS.$fileName;
			...
			$path = WWW_ROOT . 'img'.DS.'qualities' .DS. $fileName;
			$path_th = WWW_ROOT . 'img'.DS.'advans'.DS.'thumbs'.DS. $fileName;
			$path_orig = WWW_ROOT . 'img'.DS.'advans'.DS.'original'.DS. $fileName;

			$file_move = $file->moveTo($path_orig);
			$this->_resizeImg($path_orig, $path_th, 400, 400, $ext);
			$this->_resizeImg($path_orig, $path, 800, 800, $ext);
	...
	public function _setImgFirst($file){
		if( is_string($file) ){
			return $this->_customUploadImgProd($file);
		} else{
			$mimeType = $file->getClientMediaType();
			switch($mimeType){
				case("image/jpeg"):
	...
	public function _setImgSec($file){
		if( is_string($file) ){
			return $this->_customUploadImgProd($file);
		} else{
			$mimeType = $file->getClientMediaType();
