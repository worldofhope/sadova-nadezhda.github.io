...
class Comp extends Entity{
	...
	protected function _customUploadImg($file = array()){
		if( is_array($file) || is_object($file) ){
			...
			$oldPath = WWW_ROOT.'img'.DS.'comps'.DS.$fileName;
			$oldPathThumb = WWW_ROOT.'img'.DS.'comps'.DS.'thumbs'.DS.$fileName;
			...
			$path = WWW_ROOT . 'img'.DS.'comps' .DS. $fileName;
			$path_th = WWW_ROOT . 'img'.DS.'comps'.DS.'thumbs'.DS. $fileName;
			$path_orig = WWW_ROOT . 'img'.DS.'comps'.DS.'original'.DS. $fileName;

			$file_move = $file->moveTo($path_orig);
			$this->_resizeImg($path_orig, $path_th, 3000, 3000, $ext);
	...
	protected function _genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img'.DS.'comps'.DS. $name)){
