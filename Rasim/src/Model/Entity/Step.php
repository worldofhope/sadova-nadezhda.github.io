...
class Step extends Entity{
	...
	protected function _customUploadImg($file = array()){
		if( is_array($file) || is_object($file) ){
			...
			$oldPath = WWW_ROOT.'img'.DS.'steps'.DS.$fileName;
			$oldPathThumb = WWW_ROOT.'img'.DS.'steps'.DS.'thumbs'.DS.$fileName;
			...
			$path = WWW_ROOT . 'img'.DS.'steps' .DS. $fileName;
			$path_th = WWW_ROOT . 'img'.DS.'steps'.DS.'thumbs'.DS. $fileName;
			$path_orig = WWW_ROOT . 'img'.DS.'steps'.DS.'original'.DS. $fileName;
	...
	protected function _genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img'.DS.'steps'.DS. $name)){
